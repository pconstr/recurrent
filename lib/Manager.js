...
module.exports = function (queueName, port) {
  ...
  function cancel(err) {
    console.error(err);
    if(listClient.connected && zsetClient.connected) {
  ...
  var ready = false;
  function checkReady() {
    if(!ready && listClient.ready && zsetClient.ready) {
      if(that.debug) {
  ...
  that.go = function() {
    // FIXME: get rid of this, not needed any more
  };
  that.stop = function() {
    // FIXME: implement this
    listClient.removeAllListeners();
    listClient.end();
    listClient.removeAllListeners();
    zsetClient.end();
    web.close();
