...
module.exports = function (queueName, port) {
  ...
  function putBack(taskId) {
    ...
    zsetClient.watch(takenListKey, function(err, result) {
      ...
      zsetClient.lrange(takenListKey, 0, 10000, function(err, taken) {
        ...
        if(that.debug) {
          console.error('taken', taken);
        }
        if(taken.indexOf(taskId) !== -1) {
          if(that.debug) {
            console.error(taskId, 'is still taken');
          }
          ...
          zsetClient.multi()
            ...
            .rpush(pendingListKey, taskId, function(err, result) {
              ...
              if(that.debug) {
                console.error(taskId, 'back in pending queue');
              }
            })
          ...
          zsetClient.unwatch();
          if(that.debug) {
            console.error(taskId, 'is not taken...');
          }
        }
      });
    });
  ...
  function pickTimeout() {
    ...
    pick(function(nextTime, now) {
      if(that.debug) {
        console.error('nextTime = ', nextTime);
      }
      if(queuedAdds.length > 0)
        nextTime = now;
      if(nextTime !== null) {
  ...
  function pick(cb) {
    zsetClient.zrangebyscore(zsetKey, '-inf', '+inf', 'WITHSCORES', 'LIMIT', 0, 1, function(err, result) {
      ...
      if(targetTime > now) {
        if(that.debug) {
          console.error('next task', taskId, 'only at', targetTime, 'now', now);
        }
